#!/usr/bin/env python3
"""靶机 41：LSB 图片隐写 — Flag 隐藏在图片像素数据的最低有效位中。"""
from base import TargetHandler, run_server
from urllib.parse import urlparse


class Handler(TargetHandler):
    flag_locations = ["用 zsteg -a 扫描图片 LSB 提取 Flag"]

    def _send_text(self, text: str, filename: str = None):
        body = text.encode("utf-8")
        self.send_response(200)
        self.send_header("Content-Type", "text/plain; charset=utf-8")
        self.send_header("Access-Control-Allow-Origin", "*")
        if filename:
            self.send_header("Content-Disposition", f'attachment; filename="{filename}"')
        self.end_headers()
        self.wfile.write(body)

    def _build_image_data(self) -> str:
        """构造模拟图片像素数据：每行最后一个字符拼接起来就是 Flag。"""
        lines = []
        for ch in self.flag:
            line = f"PIXEL R=128 G=064 B=192 LSB=" + ch
            lines.append(line)
        lines.append("# 提示：提取每行末尾字符即可得到 Flag")
        return "\n".join(lines) + "\n"

    def handle_get(self, parsed: urlparse):
        if parsed.path == "/":
            html = """<!DOCTYPE html>
<html lang="zh-CN">
<head>
<meta charset="UTF-8">
<title>LSB 图片隐写</title>
<style>
body { font-family: sans-serif; background: #0f172a; color: #e2e8f0; max-width: 860px; margin: 0 auto; padding: 20px; }
h1 { color: #38bdf8; }
.box { background: #1e293b; padding: 20px; border-radius: 10px; margin: 15px 0; }
a.dl { display: inline-block; padding: 12px 24px; background: #38bdf8; color: #0f172a; border-radius: 6px; text-decoration: none; font-weight: bold; }
.hint { background: #1e1b0f; color: #fde68a; padding: 12px; border-radius: 6px; }
code { background: #334155; padding: 2px 6px; border-radius: 4px; }
</style>
</head>
<body>
<h1>🖼️ LSB 图片隐写分析</h1>
<div class="box">
<h3>下载隐写图片</h3>
<a class="dl" href="/image">下载 challenge_lsb.txt</a>
</div>
<div class="hint">
<strong>提示：</strong>Flag 隐藏在图片的 LSB（最低有效位）中。<br>
- 真实场景：用 <code>zsteg -a image.png</code> 扫描 LSB 提取隐藏数据；<br>
- 此处模拟：每行文本的最后一个字符拼接起来就是 Flag。
</div>
</body>
</html>"""
            self._send_html(html)
        elif parsed.path == "/image":
            self._send_text(self._build_image_data(), filename="challenge_lsb.txt")
        else:
            self._send_html("<h1>404</h1>", 404)


if __name__ == "__main__":
    import sys
    port = int(sys.argv[1]) if len(sys.argv) > 1 else 8121
    run_server(Handler, port)
